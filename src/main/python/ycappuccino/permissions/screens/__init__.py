"""
The admin console's layout (application.yml) and screens, YAML templates shared by the terminal
(frontend_shell) and the browser (frontend_web) consoles. Their endpoints name what the console wires them to: login is ILoginService's
...
from ycappuccino.ui.application import Application, load_application_yaml
...
def load_application() -> Application:
    """the console layout (application.yml) both consoles render"""
    return load_application_yaml((_SCREENS_DIR / "application.yml").read_text(encoding="utf-8"))


def load_screen(name: str) -> Screen:
    return _load(name)
